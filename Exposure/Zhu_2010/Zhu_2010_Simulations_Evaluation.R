...
load('Zhu_2010_Simulations.RData')
...
evaluation_func <- function(simulations){
  ...
  for(k in 1:length(simulations)){
    sim <- simulations[[k]]
    x <- as.numeric(sim$optimized_params)
    age <- sim$age#days
    n <- sim$n
    ...
    scores_df[k,] <- c(mean(rmse_score_per_conc), mean(AAFE_score_per_conc), mean(PBKOF_score_per_conc))  
  }
  return(scores_df)
}
...
#best with PBKOF
plot_func(output[[8]], C_water_0, nm_types, V_water,  ksed_predicted)
#best with rmse
plot_func(output[[10]], C_water_0, nm_types, V_water,  ksed_predicted)
#best with AAFE
plot_func(output[[11]], C_water_0, nm_types, V_water,  ksed_predicted)
