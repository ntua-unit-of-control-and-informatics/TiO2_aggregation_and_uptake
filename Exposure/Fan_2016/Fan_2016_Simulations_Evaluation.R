load('C:/Users/vassi/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/Fan_2016_exposure_Simulations.RData')
...
evaluation_func <- function(simulations){
  ...
  for(k in 1:length(simulations)){
    ...
    for (j in 1:length(nm_types)) {
      ...
      PBKOF_score_per_type[j] <- mean(PBKOF_score_per_conc)
    }
    scores_df[k,] <- c(mean(rmse_score_per_type), mean(AAFE_score_per_type), mean(PBKOF_score_per_type))  
  }
  return(scores_df)
}
...
# best with PBKOF
plot_func(output[[2]], C_water_0, nm_types, V_water,  ksed_predicted)
# best with rmse
