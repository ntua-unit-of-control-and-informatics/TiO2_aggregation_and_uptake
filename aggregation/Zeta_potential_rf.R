...
drops <- c("Doi","Nanoparticle", "Mean.Hydrodynamic.size.mean.(nm)", "Polydispersity.index.(PDI)", 
          ...
          "General.medium.type", "NOM.(mg/L)", "Surface.property", "Coating",
           ...
           "Concentration.of.+1.charge.ions.(mol/L)",  "FBS.(%)",  "Temperature.(oC)",  "Conductivity.(mS/cm)"   )
...
# )
# set.seed(125)
# fitRF<-train(Zeta.Potential~ ., data = Train_scaled_dmy, method="xgbTree", trControl=trainControl ,
#               tuneGrid=tunegrid , importance=TRUE,verbosity=0)
# varImp(fitRF)
## PREDICTIONS ON TRAIN SET (resubstitution)

predRF_train<-predict(fitRF, newdata=Train_scaled_dmy)*sd_y+mean_y
...
predRF_test<-predict(fitRF, newdata=Test_scaled_dmy)*sd_y+mean_y
...
COMBO_Test<-data.frame( ID = Test_id, Data= Test_data$Zeta.Potential, RF=predRF_test)
COMBO_Train<-data.frame( ID = Train_id, Data= Train_data$Zeta.Potential, RF=predRF_train)
