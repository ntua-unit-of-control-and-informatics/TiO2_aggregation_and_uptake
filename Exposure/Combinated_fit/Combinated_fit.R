...
# Calculate the dry weigh of daphnia in mg based on Pauw et al.1981 data
...
ksed_predicted <- read.csv("C:/Users/vassi/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Chen_2019/water_exposure/Chen_2019_ksed_predictions.csv")
...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dC_water <- - (a*(F_rate/1000)*(1-C_daphnia/C_max)*N_current/V_water + k_sed + ku)*C_water
    
    # Algae
    dC_algae <- ku*C_water - ke_1*C_algae
    
    # Daphnia magna
    dC_daphnia = a*(F_rate/1000)*(1-C_daphnia/C_max)*C_water/dry_weight + F_rate*C_algae - ke_2*C_daphnia 
    ...
    for (j in 1:length(nm_types)) {
      ...
      for (i in 1:3) { #loop for the 3 different concentrations
        ...
        if(exp_id==1){ #case of Chen_2019
          ...
          sub_x <- x[1:15]
          triplette_alphas <- c(sub_x[(3*j-2) : (3*j)])
        ...
        constant_params <- c("exp_id" = exp_id, "F_rate" = F_rate, "V_water" = V_water, "dry_weight" = dry_weight,
                             ...
                             'ku'=0, 'ke_1'=0)
        fitted_params <- c("a"=triplette_alphas[i], "C_max"=x[length(x)-1], "ke_2"=x[length(x)])
...
optimization <- nloptr::nloptr(x0 = x0,
                               ...
                               ub = c(rep(1, length(x0)-2),100,100),
                               opts = opts,
                               list_of_experiments=list_of_experiments)

fitted_params <- optimization$solution

alphas_Chen_2019 <- data.frame(matrix(fitted_params[1:15], byrow = T, nrow = 5) )
colnames(alphas_Chen_2019) <- c('0.1 mg/ml', '1.0 mg/ml', '10 mg/ml')
rownames(alphas_Chen_2019) <- Chen_2019_datalist$Mapping$Type

alphas_Fan_2016 <- data.frame(matrix(fitted_params[16:33], byrow = T, nrow = 6) )
colnames(alphas_Fan_2016) <- c('0.1 mg/ml', '1.0 mg/ml', '10 mg/ml')
rownames(alphas_Fan_2016) <- Fan_2016_datalist$Mapping$Type

ke_2 <- fitted_params[length(fitted_params)]
C_max <- fitted_params[length(fitted_params)-1]
