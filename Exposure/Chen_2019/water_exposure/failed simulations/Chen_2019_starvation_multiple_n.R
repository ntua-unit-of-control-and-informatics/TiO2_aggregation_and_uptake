...
x0 <- c(rep(1,5), rep(0.001, 5), rep(0.4,5), 10, 4)
...
optimization <- nloptr::nloptr(x0 = x0,
                               eval_f = obj_func,
                               lb	= c(rep(0,10),  c( 0.165, 0.148,0.143,0.150,
                                                     0.112),rep(1.5,5),5),
                               ub = c(rep(4,5), rep(0.2,5), rep(0.7,5),rep(5,5),14),
                               opts = opts,
                               C_water_0 = C_water_0,
                               nm_types = nm_types,
                               V_water = V_water,
...
params_values <- data.frame(matrix(fitted_params[1:20], nrow = 4, byrow = T)) 
colnames(params_values) <- nm_types
rownames(params_values) <- c('a', 'ke_2', 'C_sat', "n")
age <- fitted_params[21]
