...
correlation <- cor(dmy_data)
corrplot::corrplot(correlation, method="number", addCoef.col = 1,number.cex = 0.3, tl.cex = 0.3)
dev.off()
...
## PREDICTIONS ON TRAIN SET (resubstitution)
 ...
 A <- cbind(predict(fitXGB, newdata=dmy_chen)*sd_dr+mean_dr, newdata$Concentration) 
 View(A)