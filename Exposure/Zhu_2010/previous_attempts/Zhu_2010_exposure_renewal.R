...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    C_daphnia = M_daphnia/dry_weight
    #k_sed = 0
    
    # TiO2 mass in each D.magna
    dM_daphnia = a*(F_rate/1000)*(1-C_daphnia/C_sat)*C_water -
                 ke_2*C_daphnia*dry_weight  
    
    # Excreted from each D.magna
    dM_Daphnia_excreted <-  ke_2*C_daphnia*dry_weight
    ...
    Mass_balance = N*M_daphnia + M_water + M_sed
    
    
    return(list(c(dM_daphnia,dM_Daphnia_excreted, dM_water,dM_sed),
                "C_water" = C_water, 
