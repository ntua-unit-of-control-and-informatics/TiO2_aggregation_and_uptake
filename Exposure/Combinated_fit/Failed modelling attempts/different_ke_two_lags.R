    ...
    for (j in 1:length(nm_types)) {
      ...
      for (i in 1:3) { #loop for the 3 different concentrations
        ...
        constant_params <- c("exp_id" = exp_id, "F_rate" = F_rate, "V_water" = V_water, "dry_weight" = dry_weight,
                             ...
                             'ku'=0, 'ke_1'=0)
        fitted_params <- c("a"=a, "ke_2"=ke_2, "C_sat"= x[length(x)-2], "t_0"=t_0 )
...
x0 <- c(rep(1, 11), rep(0.01, 11), 0.25,0,2)
opts <- list( "algorithm" = "NLOPT_LN_NEWUOA", #"NLOPT_LN_NEWUOA", #"NLOPT_LN_SBPLX" , #"NLOPT_LN_BOBYQA" #"NLOPT_LN_COBYLA"
              "xtol_rel" = 1e-07,
              "ftol_rel" = 1e-07,
              "ftol_abs" = 0.0,
              "xtol_abs" = 0.0 ,
...
k_Chen_2019 <- c(fitted_params[6:10])
names(k_Chen_2019) <- Chen_2019_datalist$Mapping$Type

alphas_Fan_2016 <- c(fitted_params[11:16])
...
C_sat <- fitted_params[length(fitted_params)-2]
