    ...
    for (j in 1:length(nm_types)) {
      ...
      for (i in 1:3) { #loop for the 3 different concentrations
        ...
        constant_params <- c("exp_id" = exp_id, "F_rate" = F_rate, "V_water" = V_water, "dry_weight" = dry_weight,
                             ...
                             'ku'=0, 'ke_1'=0)
        fitted_params <- c("a"=a, "ke_2"=ke_2, "C_sat"= x[length(x)-1], "t_0"=x[length(x)] )
...
x0 <- c(rep(1, 11), rep(0.01, 11), 0.25,3)
...
C_sat <- fitted_params[length(fitted_params)-1]
t_0 <- fitted_params[length(fitted_params)]
