# Working directory
setwd('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Combinated_fit')
...
C1_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Chen_2019/water_exposure/data/0.1_water_exposure.csv')
...
C2_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Chen_2019/water_exposure/data/1_water_exposure.csv')
...
C3_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Chen_2019/water_exposure/data/10_water_exposure.csv')
...
ksed_predicted <- read.csv("C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Chen_2019/water_exposure/Chen_2019_ksed_predictions.csv")
ksed_predicted <- ksed_predicted[, c(1,4,6)]
colnames(ksed_predicted) <- c("Name", "Concentration_mg/L", "k_sed")

Chen_2019_datalist <- list("exp_id"=1,
                           "Mapping"=Mapping,
...
# Load the data for concentration = 0.1 mg/l
C1_uptake_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/uptake/0.1_uptake.csv')
...
# Load the data for concentration = 1.0 mg/l
C2_uptake_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/uptake/1_uptake.csv')
...
# Load the data for concentration = 10.0 mg/l
C3_uptake_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/uptake/10_uptake.csv')
...
# Load the data for concentration = 0.1 mg/l
C1_depuration_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/depuration/0.1_depuration.csv')
colnames(C1_depuration_data)[-1] <- Mapping$Type

# Load the data for concentration = 1.0 mg/l
C2_depuration_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/depuration/1_depuration.csv')
colnames(C2_depuration_data)[-1] <- Mapping$Type

# Load the data for concentration = 10.0 mg/l
C3_depuration_data <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/depuration/10_depuration.csv')
...
final_concentrations <- read.csv('C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/exposure_data/depuration/final_concentrations.csv')[2:7]
...
# Load the predicted ksed values
ksed_predicted <- read.csv("C:/Users/ptsir/Documents/GitHub/TiO2_aggregation_and_uptake/Exposure/Fan_2016/data/Fan_2016_ksed_predictions.csv")
ksed_predicted <- ksed_predicted[, c(1,4,6)]
colnames(ksed_predicted) <- c("Name", "Concentration_mg/L", "k_sed")

Fan_2016_datalist <- list("exp_id"=2,
                          "Mapping"=Mapping,
                          "dry_weight"=dry_weight,
                          "V_water"=V_water,
                          "N"=N,
...
obj_func <- function(x, list_of_experiments, metric){
  ...
  for (k in 1:length(list_of_experiments)) { # loop over the experiments from different papers
    experiment <- list_of_experiments[[k]]
    
    exp_id <- experiment$exp_id
    Mapping <- experiment$Mapping
    dry_weight <- experiment$dry_weight
    V_water <- experiment$V_water
    N <- experiment$N
    F_rate <- experiment$F_rate  
    ksed_predicted <- experiment$ksed_predicted
    C1_data <- experiment$C1_data
    C2_data <- experiment$C2_data
    C3_data <- experiment$C3_data
    ...
    for (j in 1:length(nm_types)) {
      ...
      for (i in 1:3) { #loop for the 3 different concentrations
        ...
        if(metric == "AAFE"){
          score_per_conc[i] <- AAFE(results, exp_data[,i+1]) 
        }else if (metric =="rmse"){
    ...
    score_per_experiment[k] <- mean(score_per_type)
  }
...
plot_func <- function(optimization, list_of_experiments){
  ...
  for (k in 1:length(list_of_experiments)) { # loop over the experiments from different papers
    ...
    for (j in 1:length(nm_types)) {
      ...
      color_codes <- scales::hue_pal()(3) # to return 3 color codes 
      cls <- c()  
      ...
      if(exp_id==1){
        plots_Chen_2019[[j]] <- draw_plot
      }else if(exp_id==2){
        plots_Fan_2016[[j]] <- draw_plot
      }
      
    }
  }
  
  return(list("plots_Chen_2019"=plots_Chen_2019,
              "plots_Fan_2016"=plots_Fan_2016))
}
##############################################################################


list_of_experiments <- list("Chen_2019" = Chen_2019_datalist,
                            "Fan_2016" = Fan_2016_datalist)
# fit: one alpha per nm_type (11 different nm_types)
#      C_sat

x0 <- c(rep(1, 11), rep(0.01, 11), rep(0.2, 11),0,2)
...
optimization <- nloptr::nloptr(x0 = x0,
                               eval_f = obj_func,
                               lb	= c(rep(0,11),rep(0,11), rep(0.08,11), 0,0),
                               ub = c(rep(4,11),rep(0.2,11),  rep(0.5,11), 5,5),
                               opts = opts,
...
alphas_Chen_2019 <- c(fitted_params[1:5])
names(alphas_Chen_2019) <- Chen_2019_datalist$Mapping$Type


alphas_Fan_2016 <- c(fitted_params[6:11])
names(alphas_Fan_2016) <- Fan_2016_datalist$Mapping$Type

k_Chen_2019 <- c(fitted_params[12:16])
names(k_Chen_2019) <- Chen_2019_datalist$Mapping$Type


k_Fan_2016 <- c(fitted_params[17:22])
names(k_Fan_2016) <- Fan_2016_datalist$Mapping$Type


C_sat_Chen_2019 <- c(fitted_params[23:27])
names(C_sat_Chen_2019) <- Chen_2019_datalist$Mapping$Type

C_sat_Fan_2019 <- c(fitted_params[28:33])
names(C_sat_Fan_2019) <- Fan_2016_datalist$Mapping$Type

t_0_chen <- fitted_params[length(fitted_params)-1]
t_0_fan <- fitted_params[length(fitted_params)]


plots <- plot_func(optimization, list_of_experiments)
plots$plots_Chen_2019
plots$plots_Fan_2016
