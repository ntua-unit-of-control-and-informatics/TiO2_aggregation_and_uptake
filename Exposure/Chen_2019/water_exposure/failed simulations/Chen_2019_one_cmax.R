...
ode_func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    # Weight loss under starvation (Elen et al., 1989)
    reduction <-ifelse(time_d<2, (1- 0.32*(1-exp(-36.6*time_d))), (0.682 - 0.435 * (1- 16.79*exp(-1.41*time_d))))
...
x0 <- c(rep(1,5), rep(0.001, 5), 0.2, 9, 3)
...
optimization <- nloptr::nloptr(x0 = x0,
                               eval_f = obj_func,
                               lb	= c(rep(0,10),  0.165,7,1),
                               ub = c(rep(4,5), rep(0.2,5), 0.4,14,5),
                               opts = opts,
                               C_water_0 = C_water_0,
                               nm_types = nm_types,
                               V_water = V_water,
...
params_values <- data.frame(matrix(fitted_params[1:10], nrow = 2, byrow = T)) 
colnames(params_values) <- nm_types
rownames(params_values) <- c('a', 'ke_2')
C_sat <- fitted_params[11]
age <- fitted_params[12]
beta <- fitted_params[13]
